...
When('Jack sumbits MLSS data: {string}, {int}, {int}, {float}, {float}', (name, power, weight, lactateone, lactatetwo) => {
    cy.get('[data-cy="username"]').type(`${name}`)
    cy.get('[data-cy="power"]').type(`${power}`)
    cy.get('[data-cy="lactateone"]').type(`${lactateone}`)
    cy.get('[data-cy="lactatetwo"]').type(`${lactatetwo}`)
    cy.get('[data-cy="weight"]').type(`${weight}`)
